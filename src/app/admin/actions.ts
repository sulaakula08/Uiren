...
export async function resetUserPassword(
  ...
  // Администраторам и директорам пароль так не меняют. Иначе любой, кто получил
  // панель школы, забирает себе чужой административный доступ — а вместе с ним
  // и возможность сбросить пароль кому угодно ещё.
  if (user.role === "ADMIN" || user.role === "DIRECTOR") {
    return {
      error:
        "Пароль администратора и директора так не сбрасывается — он меняется только владельцем аккаунта в настройках.",
    };
  }
