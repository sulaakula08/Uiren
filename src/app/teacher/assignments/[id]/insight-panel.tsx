...
const ROOT_CAUSE: Record<string, { label: string; hint: string; tone: string }> =
  {
    TEACHING: {
      ...
      {insight && (
        <div className="space-y-4">
          ...
          {insight.weakConcepts.length > 0 && (
            <div className="card">
              <h3 className="mb-3 text-sm font-semibold">Просевшие понятия</h3>
              ...
              <ol className="space-y-3">
                {insight.classActions.map((a, i) => (
                  <li key={i} className="flex gap-3">
                    <span className="mt-0.5 grid size-6 shrink-0 place-items-center rounded-full bg-[var(--color-brand-soft)] text-xs font-semibold text-[var(--color-brand)]">
                      {i + 1}
                    </span>
                    <div>
                ...
                {insight.atRisk.map((s) => (
                  <div key={s.studentName} className="rounded-lg border p-3">
                    ...
                    <p className="mt-2 rounded-md bg-[var(--color-canvas)] px-3 py-2 text-sm">
                      <span className="font-medium">Персональное задание: </span>
                      {s.personalTask}
                    </p>
                  </div>
                ))}
